...
class PaintApp:
    def __init__(self, root):
        ...
        self.file_menu.add_command(label="Line", command=self.set_line_drawing_tool)
        self.file_menu.add_command(label="Pencil", command=self.set_pencil_drawing_tool)
        self.file_menu.add_command(label="ARC", command=self.set_arc_drawing_tool)
        self.file_menu.add_command(label="Rectangle", command=self.set_rectangle_drawing_tool)
        self.file_menu.add_command(label="Oval", command=self.set_oval_drawing_tool)
        self.file_menu.add_command(label="Text", command=self.set_text_drawing_tool)

        self.file_menu.add_separator()
        self.file_menu.add_command(label="Quit", command=self.quit_app)

        self.the_menu.add_cascade(label="Options", menu=self.file_menu)
    ...
    def motion(self, event=None):
        if self.drawing_tool == "pencil" and self.left_button == "down":
    ...
    def rect_draw(self, event=None):
        if None not in (self.x1_line_pt, self.y1_line_pt, self.x2_line_pt, self.y2_line_pt):
            event.widget.create_rectangle(self.x1_line_pt, self.y1_line_pt, self.x2_line_pt, self.y2_line_pt, fill="red", outline="pink", width=2)
